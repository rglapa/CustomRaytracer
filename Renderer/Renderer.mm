...
static const NSUInteger maxFramesInFlight = 5;
...
@implementation Renderer
{
    ...
    NSMutableArray *_primitivewAccelerationStrctures;
...
// Create a compute pipeline state with an optional array of additional functions to link to the compute
...
- (void)createPipelines
{
    ...
    _raytracingPipeline = [self newComputePipelineStateWithFunction:raytracingFunction linkedFunctions:[intersectionFunctions allValues]];
    ...
    if(_useIntersectionFunctions) {
        ...
        // Create a table large enough to hold all the intersection functions. Metal
        ...
        for (NSUInteger geometryIndex = 0; geometryIndex < _scene.geometries.count;geometryIndex++) {
            ...
            if (geometry.intersectionFunctionName) {
                ...
                // Create a handle to the copy of the intersection function into the
...
- (void)createBuffers {
    ...
    // for each frame. The GPU reads from one chunk which the CPU writes to the next chunk.
...
#if SUPPORTS_METAL_3
        // Retrieves the list of arguments for this geometry's intersection function's resources.
...
#else
        // Create an argument encoder for this geometry's intersection function resources.
        id <MTLArgumentEncoder> encoder = [self newArgumentEncoderForResources:geometry.resources];
...
- (id <MTLAccelerationStructure>)newAccelerationStructureWithDescriptor:(MTLAccelerationStructureDescriptor *)descriptor {
    ...
    // Allocate a buffer for Metal to write the compacted accelerated structure's size info.
    ...
    // Compute and write the compacted acceleration structure sze into the buffer. You
    // must already have built acceleration structure because Metal determines the compacted
    ...
    [commandEncoder writeCompactedAccelerationStructureSize:accelerationStructure toBuffer:compactedSizeBuffer offset:0];
    ...
    // structures, such as scene geometry. Avoid compacting acceleration structures that
...
// tranformation matrix that describes where to place them in the scene.
- (void)createAccelerationStructures
{
    MTLResourceOptions options = getManagedBufferStorageMode();
    
    _primitivewAccelerationStrctures = [[NSMutableArray alloc]init];
    ...
    for (NSUInteger i = 0;i < _scene.geometries.count;i++) {
        ...
        [_primitivewAccelerationStrctures addObject:accelerationStructure];
    }
    ...
    for (NSUInteger instanceIndex = 0;instanceIndex < _scene.instances.count;instanceIndex++) {
        ...
        MTLAccelerationStructureInstanceOptionOpaque : 0;
    ...
    accelDescriptor.instancedAccelerationStructures = _primitivewAccelerationStrctures;
...
- (void)mtkView:(MTKView *)view drawableSizeWillChange:(CGSize)size
{
    ...
    // uses these values to decorrelate pixels while drawing psuedorandom numbers from the
    // Halton sequence.
...
- (void)drawInMTKView:(MTKView *)view {
    ...
    // When the GPU finishes processing the comand buffer for the frame, signal
    ...
    NSUInteger height = (NSUInteger)_size.width;
    ...
    // size, because earlier, when it created teh pipeline objects, it declared that the pipeline
    // would always use a threadgroup size that's a multiple fo the thread execution width
    // (SIMD group size). An 8x8 threadhgroup is a safe threadgroup size and small enough to be
    ...
    // Bind acceleration structure and intersection function table. Those bind to normal buffer
    // binding slots.
    ...
    // it only references thse resources indirectly via the resource buffer. Metal makes all the
    ...
    // buffer directly, so it doesn't need to mark it explictly.
    for (Geometry *geometry in _scene.geometries)
    ...
    for (id <MTLAccelerationStructure> primitiveAccelerationStructure in _primitivewAccelerationStrctures)
    ...
    if (view.currentDrawable) {
        ...
        // Drw a quad which fills the screen.
